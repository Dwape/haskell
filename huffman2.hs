...
encode s = foldr (++) [] (map (code (createCodes (buildTree (sortedCharNodes (frequency s))) [])) s)

--Returns the code for a given char
code :: [(String, Char)] -> Char -> String
code ((s, c):xs) x = if x == c then s else code xs x
...
main = do
    print (encode "")
    --print (createCodes (buildTree (sortedCharNodes (frequency "A_DEAD_DAD_CEDED_A_BAD_BABE_A_BEADED_ABACA_BED"))) [])